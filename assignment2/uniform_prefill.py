...
class Engine:
    ...
    def run(self, input_ids, prefill=True):
        ...
        for i in range(self.layers):
            ...
            q = q.view(batch_size, seq_len, self.num_qo_heads, self.head_dim).transpose(
                1, 2
            )
            ...
            attn_out = torch.matmul(attn_probs, v_expanded.to(attn_probs.dtype)).to(
                v_expanded.dtype
            )
            ...
            attn_out = (
                ...
                .contiguous()
                .view(batch_size, seq_len, self.num_qo_heads * self.head_dim)
            )
        ...
        logits = torch.nn.functional.linear(
            x[:, -1:, :], self.weights["lm_head_weight"]
        )
